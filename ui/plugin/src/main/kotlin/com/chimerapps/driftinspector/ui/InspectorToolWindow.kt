...
import com.chimerapps.driftinspector.ui.actions.NewSessionAction
import com.chimerapps.driftinspector.ui.settings.DriftInspectorSettings
import com.chimerapps.driftinspector.ui.util.adb.ADBUtils
import com.chimerapps.driftinspector.ui.util.dispatchMain
...
class InspectorToolWindow(private val project: Project, private val disposable: Disposable) :
    ...
    companion object {
        fun get(project: Project) : Pair<InspectorToolWindow, ToolWindow>? {
            ...
            val window = toolWindowManager.getToolWindow("Drift Inspector") ?: return null
    ...
    var adbInterface: ADBInterface? = null
        get() = synchronized(this@InspectorToolWindow) {
            ...
            if (!result.isRealConnection) {
                val path = DriftInspectorSettings.instance.state.adbPath
                if (path != null && File(path).let { it.exists() && it.canExecute() }) {
                    adbBootstrap = ADBBootstrap(ADBUtils.guessPaths(project)) { DriftInspectorSettings.instance.state.adbPath }
    ...
    init {
        ...
        tabsContainer = RunnerLayoutUi.Factory.getInstance(project)
            .create("drift-inspector-ui", "Detail tabs", "Some session name?", disposable)
    ...
    private fun setupViewActions(): ActionToolbar {
        ...
        val toolbar = ActionManager.getInstance().createActionToolbar("Drift Inspector", actionGroup, false)
