package com.chimerapps.driftinspector.ui.util.list

import com.chimerapps.driftinspector.ui.util.list.DiffUtil.DiffResult
import java.util.*
...
class BatchingListUpdateCallback(private val wrapped: ListUpdateCallback) : ListUpdateCallback {
    ...
    override fun onChanged(position: Int, count: Int, payload: Any?) {
        if (lastEventType == TYPE_CHANGE &&
            ...
            lastEventPosition = Math.min(position, lastEventPosition)
            lastEventCount = Math.max(previousEnd, position + count) - lastEventPosition
            return
        }
        dispatchLastEvent()
        lastEventPosition = position
...
object DiffUtil {
    ...
    private fun forward(
        ...
        val checkForSnake = Math.abs(range.oldSize() - range.newSize()) % 2 == 1
